...
export const fetchSneakers = createAsyncThunk<Sneaker[], FilterData>(
  ...
  async (filterData, { rejectWithValue }) => {
    try {
      const sizesParam = filterData.sizes
        .map((size: string) => `sizes[]=${size}`)
        .join("&");
      const genderParam = filterData.gender
        ? `&gender=${filterData.gender}`
        : "";
      const { data } = await axios.get(
        `${"https://3cf5714c82c4c5ee.mokky.dev/"}sneakers?price[from]=${
          filterData.minPrice
        }&price[to]=${filterData.maxPrice}${genderParam}&${sizesParam}&limit=20`
      );
      return data.items;
    } catch (error) {
...
export const sneakersSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      ...
      (state, action: PayloadAction<Sneaker[]>) => {
